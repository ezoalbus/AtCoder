...
bit全探索で余裕（https://drken1215.hatenablog.com/entry/2019/12/14/171657）
*/
...
int main() {
    ...
    while (cin >> N) {
        ...
        long long res = 1LL<<60;  // 2^60
        // 1 << (N - 1) = 2^{N-1}
        for (int bit = 0; bit < (1<<(N-1)); ++bit) {
            // sumは, 最小化したいすべての値のXOR
            // curは現在の区間のOR
            long long sum = 0, cur = A[0];
            // 数列の仕切りを順に探索
            for (int i = 0; i < N-1; ++i) {
                // (bit & (1 << i))はbitのi番目のフラグが立っているか否か
                // 言い換えると，整数値 bit で表される「選び方(部分集合の決め方)」において、
                //「i を選ぶかどうか」を判定する。
                if (bit & (1<<i)) sum ^= cur, cur = A[i+1];
                else cur |= A[i+1];
            }
            sum ^= cur;  // 最後の最右区間の文をsumとXOR
            chmin(res, sum);
        }
        cout << res << endl;
    }
